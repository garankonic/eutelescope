    ...
    if (_fevent) 
    {
		...
		if(alignment->getSensorID() == 32) {
		 ...
		 if(alignment->getSensorID() == 60 || alignment->getSensorID() == 61) {
			...
			EUTelAlignmentConstant * alignment1 = static_cast< EUTelAlignmentConstant * > ( _alignmentCollectionVec->getElementAt( cPlane32_Pos ) );
			alignment1->setSensorID(alignment->getSensorID());
			// apply now
